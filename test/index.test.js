...
const checkRunRerequested = JSON.parse(
  fs.readFileSync(
...
const checkRunRerunCompleted = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, "fixtures/check_run.rerun_completed.json"),
    "utf-8",
  ),
);
...
describe("My Probot app", () => {
  ...
  test("creates check run on check_run.rerequested", async () => {
    const mock = nock("https://api.github.com")
      .post("/app/installations/2/access_tokens")
      .reply(200, {
        token: "test",
      ...
      .post("/repos/hiimbex/testing-things/check-runs", (body) => {
        body.started_at = "2018-10-05T17:35:21.594Z";
        body.completed_at = "2018-10-05T17:35:53.683Z";
        assert.deepStrictEqual(body, checkRunRerunCompleted);
        return true;
      })
      .reply(200, { id: 789 });

    // Receive a check_run rerequested event
    await probot.receive({ name: "check_run", payload: checkRunRerequested });
